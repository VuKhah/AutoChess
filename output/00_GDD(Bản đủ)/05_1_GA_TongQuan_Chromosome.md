...
| **summonerBot** | `genes[14]` (eSummon), `genes[5]` (wReborn), `genes[8]` (tOnDeath), `genes[35]` (tOnAllyDeploy) cao; `genes[27]` (wProactiveSell) thấp | Summon/consume chain, giữ shell units, ưu tiên ability hơn chỉ số thuần |
| **resilientBot** | `genes[1]` (wHP), `genes[4]` (wTaunt), `genes[5]` (wReborn), `genes[6]` (wSafeguard), `genes[17]` (eGiveBuff) cao; `genes[0]` (wATK) thấp | Phòng thủ bền bỉ, hút damage, buff đồng minh, không ưu tiên tấn công |
...
**Gene[34] tOnAllyGroup** là gene quan trọng nhất trong nhóm: nó bao phủ ba trigger liên quan đến "đồng minh trong combat" — OnAllyDeath, OnAllySummon, OnAllyReborn. Ba trigger này thường đi cùng nhau (Anubis, Osiris, Sobek, Isis đều dùng ít nhất một trong số đó), và cho phép GA gán một trọng số duy nhất phản ánh mức độ bot quan tâm đến "chain combat events".

**Gene[35] tOnAllyDeploy** tách biệt hẳn khỏi nhóm vì OnAllyDeploy xảy ra trong *Shop Phase*, không phải combat — đây là trigger phản ánh hành vi "mỗi khi đặt thêm đồng minh lên sân". Kiểu chiến lược tận dụng trigger này (deploy nhiều unit mỗi lượt để tích buff) khác hẳn với chiến lược summon chain trong combat, nên GA cần gene độc lập để học. summonerBot được seed `genes[35] ∈ [0.50, 0.85]` — phản ánh archetype cần cả deploy trigger (shop) lẫn death/summon chain (combat).
