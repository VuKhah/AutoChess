...
**Thứ ba: Pipeline headless training khép kín trên phần cứng cá nhân.** Toàn bộ chuỗi từ `train_ai.ps1` → `GATrainer` → `GameSimulator` (hơn 2 triệu trận đấu ở production mode) → `AI_Library.json` → game runtime hoàn thành trong 20–30 phút mà không cần GPU, không cần cloud, không cần bất kỳ thao tác thủ công nào. Đây là kết quả của quyết định kiến trúc nhất quán từ đầu: ranh giới cứng giữa plain C# (Core Engine, AI) và MonoBehaviour (Manager, UI), với toàn bộ game logic tồn tại độc lập với Unity runtime.

Ba đóng góp này không độc lập mà tạo thành một chuỗi phụ thuộc: TTE engine cung cấp ngôn ngữ để mô tả hành vi game; chromosome 37-gene học cách *đánh giá* ngôn ngữ đó qua các gene trigger/effect; headless training đặt toàn bộ quá trình học vào thực tiễn trong thời gian đủ ngắn để iterate nhiều lần. Thiếu một trong ba, hệ thống không thành hình — TTE mà không có AI learning chỉ là engine có nhiều lá bài; chromosome mà không có môi trường đủ trung thực sẽ học chiến lược sai; training hiệu quả mà không có game đủ sâu sẽ ra bot không có gì thực sự để học. Đóng góp của dự án, nói cho đúng, là sự *tích hợp* của ba yếu tố đó thành một hệ thống hoạt động được.
...
Cuối cùng, dự án này là bằng chứng rằng nghiên cứu AI trong game không cần hàng tỷ tham số hay data center để tạo ra kết quả thú vị. Với 37 con số, một fitness function được thiết kế cẩn thận, và hơn 2 triệu trận đấu mô phỏng trong 30 phút, Genetic Algorithm có thể tạo ra những bot chơi Auto Chess theo cách đủ đa dạng để người chơi thật nhận ra và cảm nhận sự khác biệt — babylonBot tích lũy chậm nhưng chắc, summonerBot bất ngờ với summon chain, resilientBot kiên cường qua những trận tưởng đã thua. Sự đơn giản có chủ ý — 37 gene thay vì triệu tham số — không phải là giới hạn của dự án; nó là điều kiện cho phép toàn bộ hệ thống tồn tại, được phân tích, và được tái hiện trên phần cứng bình thường.
