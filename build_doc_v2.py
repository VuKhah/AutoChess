"""
build_doc_v2.py — Ghép 20 file GDD (đã tái cấu trúc) thành master_v2.md rồi dùng Pandoc tạo TieuLuan_AutoChess_GA_v2.docx
Chạy: python build_doc_v2.py
"""
import subprocess, sys, os

BASE   = "Document/00_GDD"
OUT_MD = "output/master_v2.md"
OUT_DOCX = "output/TieuLuan_AutoChess_GA_v2.docx"
...
def build_master():
    ...
    with open(OUT_MD, encoding="utf-8") as f:
        content = f.read()
    print(f"[1/3] master_v2.md done ({len(content):,} chars, {content.count(chr(92)+'newpage')} page breaks)")


def make_reference():
    """Tạo reference.docx từ pandoc default rồi patch bằng python-docx (nếu chưa có thì tạo mới)."""
    if os.path.exists(REF_DOCX):
        print(f"[2/3] Tái sử dụng reference.docx có sẵn")
        return True
...
if __name__ == "__main__":
    ...
    run_pandoc(has_ref)
    print("\nHoàn tất. Mở output/TieuLuan_AutoChess_GA_v2.docx để kiểm tra.")
