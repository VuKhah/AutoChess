...
Trong đề tài, hàm fitness được đo qua **số điểm tích lũy từ các trận đấu mô phỏng**: mỗi chromosome thi đấu `matchesPerChrom` trận (5 trận quick mode, 20 trận production mode) với các đối thủ chọn ngẫu nhiên từ quần thể. Mỗi trận thắng tích 10 điểm, mỗi trận hòa tích 2 điểm, thua được 0:

```
fitness(c) = Σᵢ score(trận i),   i ∈ [0, matchesPerChrom)
```

Có một chi tiết thiết kế đáng chú ý: điểm hòa được đặt là 2, không phải 0 hay 1. Nếu hòa = 0, chromosome có chiến lược phòng thủ bền bỉ (không thắng nhanh nhưng cũng không thua nhanh) sẽ không bao giờ được chọn, dù đó có thể là chiến lược tốt trong nhiều tình huống. Ngược lại nếu hòa = 5 (bằng nửa điểm thắng), GA có xu hướng hội tụ về chiến lược "chơi an toàn" không có rủi ro nhưng cũng không thắng. Giá trị 2 được chọn sau thực nghiệm để cân bằng: thưởng cho sự bền bỉ, nhưng không bao giờ để hòa cạnh tranh với chiến lược thắng thực sự.
...
Để chống lại premature convergence, đề tài áp dụng đồng thời hai chiến lược bổ trợ nhau.
...
Chiến lược thứ hai là **immigration**: mỗi thế hệ, `immigrantRate = 12%` cá thể trong thế hệ mới là các chromosome seeded được tạo mới hoàn toàn (không phải con của bất kỳ cá thể cũ nào). Cơ chế này liên tục "bơm" vật liệu di truyền mới vào quần thể, ngăn chặn hiện tượng *genetic drift* — sự mất đa dạng do ngẫu nhiên thuần túy dù không có áp lực chọn lọc. Hơn nữa, nếu hệ thống phát hiện một bộ tộc bị underrepresented (dưới 10% quần thể), số immigrant cho bộ tộc đó được tăng thêm 2 để phục hồi:

```csharp
if (pctBabylon < 10f) immigrantCount += 2;  // bơm thêm Babylon nếu thiếu
if (pctNiles   < 10f) immigrantCount += 2;  // bơm thêm Niles nếu thiếu
```

Cuối cùng, hệ thống theo dõi std_dev liên tục và dừng sớm nếu phát hiện quần thể đã hội tụ (plateau 15 thế hệ liên tiếp với thay đổi std_dev < 0.5) — tránh lãng phí thời gian tính toán khi không còn cải thiện có thể xảy ra:

```csharp
const int   PLATEAU_PATIENCE = 15;
const float PLATEAU_EPS      = 0.5f;

if (Mathf.Abs(stdDev - prevStdDev) < PLATEAU_EPS)
    plateauCount++;
else
    plateauCount = 0;
...
{
    Debug.LogWarning($"Early stop tại gen {g} — plateau {PLATEAU_PATIENCE} gen liên tiếp.");
    break;
}
```
...
Phần tiếp theo (2.2) sẽ trình bày nền tảng kỹ thuật thứ hai của dự án: kiến trúc Unity Engine và các nguyên tắc lập trình game cần thiết để xây dựng môi trường mà hệ thống AI vừa mô tả có thể chạy trong.
